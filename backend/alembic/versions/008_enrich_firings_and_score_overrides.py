...
def upgrade() -> None:
    ...
    for column_name, column_type in (
        ...
        ("recency", sa.Float()),
        ("relevance", sa.Float()),
        ("spread_boost", sa.Float()),
        ("prompt_position", sa.Integer()),
        ("was_included", sa.Boolean()),
    ):
        if not _column_exists("neuron_firings", column_name):
            op.add_column("neuron_firings", sa.Column(column_name, column_type, nullable=True))

    # --- neuron_score_overrides table ---
    if not _table_exists("neuron_score_overrides"):
        op.create_table(
...
def downgrade() -> None:
    if _table_exists("neuron_score_overrides"):
    ...
    for col in [
        ...
        if _column_exists("neuron_firings", col):
