...
class AgencyPolicy(Base):
    """Immutable version of an incentive policy once it leaves draft."""
    __tablename__ = "agency_policies"
    __table_args__ = (Index("ix_agency_policy_name_version", "name", "version", unique=True),)
    ...
    mode: Mapped[str] = mapped_column(String(20), nullable=False, default="simulation", server_default="simulation")
    config: Mapped[dict] = mapped_column(JSONB, nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_by: Mapped[str] = mapped_column(String(100), nullable=False, default="user", server_default="user")
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now())
    activated_at: Mapped[datetime.datetime | None] = mapped_column(DateTime, nullable=True)


class AgencyWorkerProfile(Base):
    """A scored actor is the entire model+harness+role+skills configuration."""
    __tablename__ = "agency_worker_profiles"
    ...
    harness: Mapped[str] = mapped_column(String(60), nullable=False, index=True)
    task_classes: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    risk_tiers: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    skill_ids: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    permission_ceiling: Mapped[str] = mapped_column(String(50), nullable=False, default="project_reversible")
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="active", server_default="active")
    agency_capital: Mapped[float] = mapped_column(Float, nullable=False, default=100.0, server_default="100")
    peak_capital: Mapped[float] = mapped_column(Float, nullable=False, default=100.0, server_default="100")
    beta_alpha: Mapped[float] = mapped_column(Float, nullable=False, default=2.0, server_default="2")
    beta_beta: Mapped[float] = mapped_column(Float, nullable=False, default=2.0, server_default="2")
    score_totals: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())


class AgencyExperiment(Base):
    """A focused causal comparison blocked by model/harness/task/risk."""
    __tablename__ = "agency_experiments"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="draft", server_default="draft")
    mode: Mapped[str] = mapped_column(String(20), nullable=False, default="simulation", server_default="simulation")
    policy_ids: Mapped[list] = mapped_column(JSONB, nullable=False)
    ...
    sample_target: Mapped[int] = mapped_column(Integer, nullable=False, default=30)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now())


class AgencyScoreEvent(Base):
    """Append-only behavioral/outcome event; adjustments are new events."""
    __tablename__ = "agency_score_events"
    __table_args__ = (Index("ix_agency_score_worker_time", "worker_profile_id", "created_at"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    worker_profile_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_worker_profiles.id"), nullable=False, index=True)
    policy_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_policies.id"), nullable=False, index=True)
    experiment_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("agency_experiments.id"), nullable=True, index=True)
    work_order_id: Mapped[str | None] = mapped_column(String(120), nullable=True, index=True)
    event_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    task_class: Mapped[str] = mapped_column(String(80), nullable=False)
    risk_tier: Mapped[int] = mapped_column(Integer, nullable=False)
    dimensions: Mapped[dict] = mapped_column(JSONB, nullable=False)
    points_total: Mapped[float] = mapped_column(Float, nullable=False)
    settled_points: Mapped[float] = mapped_column(Float, nullable=False)
    escrow_points: Mapped[float] = mapped_column(Float, nullable=False)
    ...
    adjustment_of_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("agency_score_events.id"), nullable=True)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now(), index=True)


class AgencyCapitalTransaction(Base):
    """Immutable agency-capital flight recorder."""
    __tablename__ = "agency_capital_transactions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    worker_profile_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_worker_profiles.id"), nullable=False, index=True)
    score_event_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("agency_score_events.id"), nullable=True)
...
class AgencyVenturePlan(Base):
    """North-star intent. Agents execute projections, never mutate this row in place."""
    __tablename__ = "agency_venture_plans"
...
class AgencyPlanRevision(Base):
    """Immutable constitutional graph revision for a venture."""
    __tablename__ = "agency_plan_revisions"
    __table_args__ = (Index("ix_agency_plan_revision", "venture_plan_id", "revision", unique=True),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    venture_plan_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_venture_plans.id"), nullable=False, index=True)
...
class AgencyWorkOrder(Base):
    """Disposable execution projection derived from a pinned north-star revision."""
    __tablename__ = "agency_work_orders"

    id: Mapped[str] = mapped_column(String(120), primary_key=True)
    venture_plan_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_venture_plans.id"), nullable=False, index=True)
    plan_revision: Mapped[int] = mapped_column(Integer, nullable=False)
    plan_node_id: Mapped[str] = mapped_column(String(120), nullable=False, index=True)
    worker_profile_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("agency_worker_profiles.id"), nullable=True, index=True)
    policy_id: Mapped[int] = mapped_column(Integer, ForeignKey("agency_policies.id"), nullable=False)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="issued", server_default="issued", index=True)
    task_class: Mapped[str] = mapped_column(String(80), nullable=False)
    risk_tier: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    locked_completion: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    audit_state: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=func.now())
    completed_at: Mapped[datetime.datetime | None] = mapped_column(DateTime, nullable=True)


class AgencyPermissionLease(Base):
    """Time- and scope-bounded authority granted to one work order."""
    __tablename__ = "agency_permission_leases"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    work_order_id: Mapped[str] = mapped_column(String(120), ForeignKey("agency_work_orders.id"), nullable=False, index=True)
...
class AgencyWorkOrderEvent(Base):
    """Append-only flight recorder for contract delivery, claims, audits, and invalidation."""
    __tablename__ = "agency_work_order_events"
