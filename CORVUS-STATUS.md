...
- **kill-locomo-bench — [PARITY, preliminary], done for conv 0.** Harness at `eval/locomo/run_locomo.py` (session-by-session Opus distill through the production write gate into throwaway `corvus-locomo` tenant; recall-only answering; Sonnet LLM judge). Conversation 0, all 199 questions:

  | condition | overall | multi-hop | temporal | open-domain | single-hop | adversarial |
  |---|---|---|---|---|---|---|
  | Corvus memory (spread on) | **62.8%** | 40.6 | 67.6 | 38.5 | 57.1 | 89.4 |
  | spread disabled (ablation) | **64.3%** | 43.8 | 67.6 | 38.5 | 58.6 | 91.5 |
  | full-context baseline | **68.8%** | 62.5 | 59.5 | 46.2 | 88.6 | 57.4 |

  Published comparisons: Mem0 67.13% LLM-judge (their paper, all 10 convs); Zep contested — 65.99% per Mem0, 75.14% per Zep's rebuttal, 58.44% per Mem0's re-correction of Zep's own pipeline. Our 62.8% on 1/10 convs sits inside the published band (above Zep-corrected, below Mem0) with a deliberately conservative extractor (~8 facts/session; single-hop coverage is the visible cost — 57.1 vs the baseline's 88.6). Memory BEATS full-context on temporal (+8.1pp) and adversarial (+32pp — the strict answer prompt refuses to hallucinate). Full 10-conv sweep is a documented command away but hours of CLI time; conv-0 numbers stand as preliminary.
- **kill-graph-retrieval — [PARITY; BEAT not demonstrated], done.** The measured ablation: disabling spread changed overall accuracy −1.5pp *in spread's disfavor* (62.8 vs 64.3, 3 questions, within noise). Mechanistic caveat: the fresh eval tenant had only 39 promoted edges — co-fire edge accumulation needs operational history, so spread was nearly inert. Structural parity with Zep/Mem0 graph retrieval remains true (recall runs the full spread+inhibition pipeline), but no public claim of graph advantage is permitted until a measurement on an edge-rich graph shows it.
- **kill-framework-sdks — [PARITY], done.** `integrations/crewai/`: corvus_recall/corvus_remember tools + write-gated StorageBackend, verified live under crewai 1.15.2. Found the wedge: CrewAI 1.x's `StorageBackend.search()` passes only the query embedding, locking out every remote text-pipeline memory — the additive upstream fix (query text alongside embedding) is the PR to lead the contribution strategy with.
